...
- **`output_video_generation` on-demand module (Phase B modularization)**: Video Generation (hyperframes + Node) is now a downloadable runtime module. `resolve_node_binary_path(app)` and `resolve_hyperframes_cwd(app)` in `paths.rs` gained an `AppHandle` parameter and check `%LOCALAPPDATA%\Trispr Flow\modules\output_video_generation\bin\` before falling back to bundled/system paths — one call site each in `video_generation.rs::run_hyperframes_render_inner` updated. Registry entry flipped from `installed_by_default: true` to `false` (Phase 1b). Note: Node.js is not bundled — users need Node 22+ on PATH or `settings.video_generation.node_binary_path` configured. Package manifest at `module-sidecars/video_gen/trispr-module.json`; build script at `scripts/windows/build-video-gen-module.ps1`.

- **`piper_tts` on-demand module (Phase A modularization)**: Piper TTS runtime is now distributable as a downloadable module package (`piper_tts`). `resolve_piper_binary` and `resolve_piper_model_dir` in `multimodal_io.rs` check `%LOCALAPPDATA%\Trispr Flow\modules\piper_tts\bin\piper\` before falling back to the bundled path — zero call-site changes, the full `speak_piper → daemon_config_from_request → resolve_piper_binary_for_runtime → resolve_piper_binary` chain benefits transparently, including the health-check path at l.1687. `piper_tts` registered in the module registry (`bundled: false`, `installed_by_default: false`). Package manifest at `module-sidecars/piper_tts/trispr-module.json` (`kind: runtime`, entrypoint `bin/piper/piper.exe`, required assets: onnxruntime DLLs, espeak-ng-data, libtashkeel_model.ort). Build/packaging script at `scripts/windows/build-piper-module.ps1` (reads version from manifest, copies pre-built binaries, bundles voices if present, zips, computes SHA256, emits `modules-index.json`; does **not** publish — see `docs/MODULE-PUBLISHING.md`). Bundled voices (`de_DE-thorsten-medium`, `en_US-amy-medium`) included in the package when present in `src-tauri/bin/piper/voices/`.
...
- **`opus` module registration + packaging**: registered the `opus` on-demand module in the registry (`bundled: false`, `installed_by_default: false`) so downloaded `opus` packages pass manifest validation. Added the package manifest (`module-sidecars/opus/trispr-module.json`, `kind: sidecar`, `entrypoint: bin/trispr-opus.exe`, FFmpeg as a required asset), a build/packaging script (`scripts/windows/build-opus-module.ps1` — builds the sidecar, bundles FFmpeg, zips, computes SHA256, emits `modules-index.json`; does **not** publish), and a publishing guide (`docs/MODULE-PUBLISHING.md`). Package verified at ~32 MB with the expected layout; publishing the release asset is a separate manual step.
- **Modules Hub: download / update / uninstall for on-demand modules**: the Hub now annotates modules against the remote `modules-index.json` (best-effort — an unreachable index leaves the registry view intact). Not-installed modules that exist in the index get a **Download (size)** button wired to `download_module`; a live `module:download-progress` listener updates the button (Downloading % → Verifying → Installing). Installed on-demand modules gain an **Uninstall** button (`uninstall_module`, with confirm) and, when the index lists a newer version, an **Update to vX** button. This completes the user-facing loop for the on-demand module delivery backbone — e.g. installing the `opus` export module from the UI.
