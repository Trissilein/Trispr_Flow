...
fn migrate_cloud_fallback_to_ai_fallback(old: &Value) -> AIFallbackSettings {
    if let Some(enabled) = old.get("cloud_fallback").and_then(|v| v.as_bool()) {
        if enabled {
...
### Breaking Changes
- `cloud_fallback` → `ai_fallback.enabled`
...
2. `cloud_fallback` flag auto-migrated into `ai_fallback.enabled`
3. AI Fallback disabled by default (user must re-enable)
4. First-time users: no existing provider config, clean setup
...
- Current implementation: `src-tauri/src/transcription.rs` (`transcribe_cloud`, to be refactored)
- Provider APIs:
