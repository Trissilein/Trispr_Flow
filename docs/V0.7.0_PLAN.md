...
**Existing Cloud Fallback** (`src-tauri/src/transcription.rs`):
- Single endpoint from env var: `TRISPR_CLOUD_ENDPOINT`
- Used as optional fallback for transcription stage
- Settings: `cloud_fallback: bool`
- Auth optional via env var `TRISPR_CLOUD_TOKEN`
...
5. Single global endpoint and toggle are too limited for multi-provider UX
...
**Decision**: ✅ **Use "AI Fallback"** throughout UI/docs
...
Settings → Post-Processing:
  ▼ Post-Processing Rules (existing)
  ▼ AI Fallback (NEW)
```
- **Pro**: Logical flow: local rules → AI refinement
...
**Decision**: ✅ **Add AI Fallback expander to Post-Processing panel**
...
Fallback:
  → settings.json (with security warning)
...
```
┌────────────────────────────────────────┐
│ Edit Custom Refinement Prompt          │
├────────────────────────────────────────┤
│ [Use Default] [Reset to Default]       │
│                                        │
│ ┌──────────────────────────────────┐   │
│ │ You are a professional           │   │
│ │ transcriptionist. The following  │   │
│ │ text is a raw transcript from    │   │
│ │ speech-to-text. Fix any errors,  │   │
│ │ improve punctuation, and ensure  │   │
│ │ proper formatting. Keep the same │   │
│ │ meaning and tone.                │   │
│ │                                  │   │
...
- Store in settings.json with warning:
  ⚠️ "API key stored in plain text. Restrict file permissions to 0600."
...
- Error handling (provider-specific errors → user-friendly messages)
...
**Task 38**: E2E testing
- Record raw transcript → refine with each provider → verify output
...
|------|-------|-------|-------|--------|
| 1 | F | Haiku | 39 | 📋 Planning (current) |
| 2-3 | G | Opus | 31, 36, 37 | 🔵 Upcoming |
| 4-5 | H | Sonnet | 32-35, 38 | 🔵 Upcoming |
...
- Current: `src-tauri/src/transcription.rs` (`transcribe_cloud`)
...
1. ✅ **Terminology**: "AI Fallback" approved?
2. ✅ **Settings location**: Expander in Post-Processing approved?
3. ✅ **Keyring integration**: Priority for Windows/macOS, fallback to file?
4. ✅ **Custom prompts**: Allow user editing, or restrict to predefined templates?
5. ⚠️ **Cost tracking**: Show estimated cost per refinement in UI?
6. ⚠️ **Streaming**: Show live refinement as it happens, or just final result?

---

**Next Step**: Block G implementation (Opus Sprint) → Architecture design
