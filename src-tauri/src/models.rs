...
mod tests {
    ...
    fn validate_model_url_rejects_userinfo() {
        let url =
            "https://user:pass@huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-large-v3.bin";
        assert!(validate_model_url(url, UrlSafety::Basic).is_err());
    }
...
pub(crate) fn resolve_model_path(app: &AppHandle, model_id: &str) -> Option<PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        let search_dirs = [cwd.join("models")];
...
fn resolve_model_path_by_file(app: &AppHandle, file_name: &str) -> Option<PathBuf> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            let exe_search_dirs = [exe_dir.join("models"), exe_dir.join("../models")];
            for dir in &exe_search_dirs {
                let candidate = dir.join(file_name);
                if candidate.exists() {
                    return Some(candidate);
...
fn load_custom_source_models(custom_url: &str) -> Result<Vec<SourceModel>, String> {
    ...
    if custom_url.ends_with(".bin") || custom_url.ends_with(".gguf") {
        let file_name =
            filename_from_url(custom_url).ok_or_else(|| "Invalid model URL".to_string())?;
    ...
    if let Ok(entries) = serde_json::from_str::<Vec<ModelIndexEntry>>(&body) {
        ...
        for entry in entries {
            ...
            let download_url = entry.url.clone().unwrap_or_else(|| {
                custom_url.trim_end_matches('/').to_string() + "/" + &entry.file_name
            });
        ...
        for entry in entries.flatten() {
            ...
            let label = match file_name.as_str() {
                ...
                "ggml-large-v3-turbo-german-q5_0.bin" => {
                    "Whisper large-v3-turbo DE (q5_0)".to_string()
                }
                "ggml-large-v3-turbo-german-q8_0.bin" => {
                    "Whisper large-v3-turbo DE (q8_0)".to_string()
                }
...
pub(crate) fn quantize_model(
    ...
    let output_name = format!("{}-{}.bin", file_name.trim_end_matches(".bin"), quant_type);
    ...
    let quantize_path = resolve_quantize_path(&app).ok_or_else(|| {
        "quantize.exe not found. Install/bundle it or set TRISPR_WHISPER_QUANTIZE.".to_string()
    })?;
    ...
    let expected_ratio = if quant_type == "q8_0" {
        0.85f64
    } else {
        0.55f64
    };
            ...
            Ok(None) => {
                if let Ok(meta) = fs::metadata(&output_path) {
                    if meta.len() > 0 && expected_output_bytes > 0 {
                        let approx_percent = ((meta.len() as f64 / expected_output_bytes as f64)
                            * 100.0)
                            .round()
...
fn apply_quantize_windows_env(cmd: &mut std::process::Command, app: &AppHandle) {
    ...
    if let Some(quantize_dir) =
        resolve_quantize_path(app).and_then(|p| p.parent().map(|d| d.to_path_buf()))
    {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        push_dir_if_exists(
            &mut dll_dirs,
            &mut seen_dirs,
            cwd.join("src-tauri/bin/cuda"),
        );
        push_dir_if_exists(
            &mut dll_dirs,
            &mut seen_dirs,
            cwd.join("src-tauri/bin/vulkan"),
        );
...
fn download_model_file(
    ...
    let result = (|| -> Result<PathBuf, String> {
        ...
        if let Some(expected_hash) = lookup_model_checksum(file_name) {
            ...
            warn!(
                "No checksum available for {}: skipping integrity check",
                file_name
            );
        }
