...
impl Default for Settings {
  fn default() -> Self {
    Settings {
      ...
      hallucination_rms_threshold: crate::HALLUCINATION_RMS_THRESHOLD,
      hallucination_max_duration_ms: crate::HALLUCINATION_MAX_DURATION_MS,
      hallucination_max_words: crate::HALLUCINATION_MAX_WORDS as u32,
      hallucination_max_chars: crate::HALLUCINATION_MAX_CHARS as u32,
    }
  }
}
...
pub(crate) fn sync_model_dir_env(settings: &Settings) {
  if !settings.model_storage_dir.trim().is_empty() {
    std::env::set_var("TRISPR_WHISPER_MODEL_DIR", settings.model_storage_dir.trim());
  } else {
...
pub(crate) fn build_overlay_settings(settings: &Settings) -> overlay::OverlaySettings {
