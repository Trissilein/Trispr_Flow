...
describe("ai-refinement settings", () => {
    ...
    it("dirty prompt stays dirty after blur (Save reachable)", () => { renderAIFallbackSettingsUi(); const t = byId<HTMLTextAreaElement>("ai-fallback-custom-prompt"); t.focus(); t.value = "edited"; t.classList.add("has-unsaved-edits"); t.blur(); renderAIFallbackSettingsUi(); expect(byId<HTMLButtonElement>("ai-fallback-prompt-preset-save").disabled).toBe(false); expect(byId<HTMLButtonElement>("ai-fallback-prompt-preset-discard").hidden).toBe(false); });
    it("reset visible but disabled for pristine built-in", () => { const t = byId<HTMLTextAreaElement>("ai-fallback-custom-prompt"); t.blur(); t.value = ""; t.classList.remove("has-unsaved-edits"); renderAIFallbackSettingsUi(); const btn = byId<HTMLButtonElement>("ai-fallback-prompt-preset-reset"); expect(btn.hidden).toBe(false); expect(btn.disabled).toBe(true); });
    it("reset enabled when built-in override exists", () => { setSettings(mkSettings({ ai_fallback: { ...settings!.ai_fallback, active_prompt_preset_id: "wording", prompt_preset_overrides: { wording: "my override" } } })); const t = byId<HTMLTextAreaElement>("ai-fallback-custom-prompt"); t.blur(); t.value = ""; t.classList.remove("has-unsaved-edits"); renderAIFallbackSettingsUi(); const btn = byId<HTMLButtonElement>("ai-fallback-prompt-preset-reset"); expect(btn.hidden).toBe(false); expect(btn.disabled).toBe(false); });
    it("reset enabled with unsaved built-in edits", () => { renderAIFallbackSettingsUi(); const t = byId<HTMLTextAreaElement>("ai-fallback-custom-prompt"); t.focus(); t.value = "edited"; renderAIFallbackSettingsUi(); const btn = byId<HTMLButtonElement>("ai-fallback-prompt-preset-reset"); expect(btn.hidden).toBe(false); expect(btn.disabled).toBe(false); });
    it("reset hidden for user preset", () => { setSettings(mkSettings({ ai_fallback: { ...settings!.ai_fallback, prompt_presets: [{ id: "u1", name: "User", prompt: "X" }], active_prompt_preset_id: "user:u1" } })); renderAIFallbackSettingsUi(); expect(byId<HTMLButtonElement>("ai-fallback-prompt-preset-reset").hidden).toBe(true); });
    it("user preset edit enables Save (dirty vs saved prompt, not live custom_prompt)", () => { setSettings(mkSettings({ ai_fallback: { ...settings!.ai_fallback, prompt_presets: [{ id: "u1", name: "User", prompt: "saved prompt" }], active_prompt_preset_id: "user:u1", custom_prompt: "edited live" } })); const t = byId<HTMLTextAreaElement>("ai-fallback-custom-prompt"); t.focus(); t.value = "edited live"; renderAIFallbackSettingsUi(); expect(byId<HTMLButtonElement>("ai-fallback-prompt-preset-save").disabled).toBe(false); });
