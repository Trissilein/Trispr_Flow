...
describe("export serialization", () => {
  ...
  describe("buildExportText - TXT format", () => {
    it("generates valid TXT export with sections and relative offsets", () => {
      const txt = buildExportText(mockEntries, "txt");

      // First section starts with date + time
      expect(txt).toMatch(/^\d{4}-\d{2}-\d{2} — \d{2}:\d{2}:\d{2}/);
      expect(txt).toContain("─");

      // Check entries with relative offsets
      expect(txt).toContain("[+00:00] Input: First entry from microphone");
      expect(txt).toContain("[+00:01] System audio: Second entry from system audio");
      expect(txt).toContain("[+00:02] Input: Third entry from microphone");

      // Check chronological order (oldest to newest)
      const firstIdx = txt.indexOf("First entry");
      const secondIdx = txt.indexOf("Second entry");
      const thirdIdx = txt.indexOf("Third entry");
  ...
  describe("buildExportText - Markdown format", () => {
    it("generates valid Markdown export with sections", () => {
      ...
      // Check section + table rows
      expect(md).toMatch(/## \d{4}-\d{2}-\d{2} — \d{2}:\d{2}:\d{2}/);
      expect(md).toContain("| Zeit | Sprecher | Text |");
      expect(md).toContain("| +00:00 | Input | First entry from microphone |");
      expect(md).toContain("| +00:01 | System audio | Second entry from system audio |");
      expect(md).toContain("| +00:02 | Input | Third entry from microphone |");

      // Check footer
      expect(md).toContain("*Generated by Trispr Flow*");
    });

    it("keeps mixed speakers in one chronological section when no long pause exists", () => {
      const md = buildExportText(mockEntries, "md");
      const sectionHeaders = md.match(/^## /gm) ?? [];
    ...
    it("handles only input entries", () => {
      ...
      expect(md).toContain("| +00:00 | Input | Test |");
      expect(md).not.toContain("| +00:00 | System audio |");
    });
    ...
    it("handles only output entries", () => {
      ...
      expect(md).toContain("| +00:00 | System audio | Test |");
      expect(md).not.toContain("| +00:00 | Input |");
    });
  });
  ...
  describe("buildExportText - JSON format", () => {
    it("generates valid JSON with correct structure", () => {
      ...
      expect(json).toHaveProperty("format_version", "2.0");
    ...
    it("includes all entry fields in JSON format", () => {
      ...
      expect(firstEntry).toHaveProperty("speaker_name", "Input");
      expect(firstEntry).toHaveProperty("text", "First entry from microphone");
      expect(firstEntry).toHaveProperty("refined_text", null);
  ...
  describe("edge cases", () => {
    it("handles special characters in text", () => {
      const specialEntries: HistoryEntry[] = [
        ...
        {
          id: "2",
          text: "Text with <html> & special chars: é, ñ, ü",
          timestamp_ms: 2000,
          source: "output",
        },
      ];
  ...
  describe("End-to-End Integration Tests", () => {
    describe("Complete Export Workflow", () => {
      it("exports realistic conversation in all formats", () => {
        ...
        expect(txtExport).toMatch(/^\d{4}-\d{2}-\d{2} — \d{2}:\d{2}:\d{2}/);
        ...
        expect(mdExport).toContain("**Entries**: 5");
        expect(mdExport).toContain("| Zeit | Sprecher | Text |");
        expect(mdExport).toContain("| +00:00 | Input | Hello, I need help with a code bug in my API. |");
        ...
        expect(parsed.entries[0].speaker_name).toBe("Input");
        expect(parsed.entries[1].speaker_name).toBe("System audio");
      ...
      it("handles mixed sources correctly", () => {
        ...
        expect(txtExport).toContain("System audio:");
    ...
    describe("Full Pipeline: Capture → Process → Export", () => {
    ...
    describe("Error Recovery and Edge Cases", () => {
      ...
      it("handles UTF-8 special characters correctly", () => {
        const entries: HistoryEntry[] = [
          { id: "1", text: "Emoji test: 🎤 🔊 📝", timestamp_ms: 1000, source: "mic" },
          { id: "2", text: "Unicode: こんにちは 你好 مرحبا", timestamp_ms: 2000, source: "mic" },
          { id: "3", text: "Symbols: € £ ¥ © ®", timestamp_ms: 3000, source: "mic" },
        ];

        const txtExport = buildExportText(entries, "txt");
        expect(txtExport).toContain("🎤 🔊 📝");
        expect(txtExport).toContain("こんにちは");
        expect(txtExport).toContain("€ £ ¥");

        const jsonExport = buildExportText(entries, "json");
        const parsed = JSON.parse(jsonExport);
        expect(parsed.entries[0].text).toContain("🎤");
        expect(parsed.entries[1].text).toContain("こんにちは");
      });
      ...
      it("handles conversation with only input entries", () => {
        ...
        expect(mdExport).toContain("| +00:00 | Input | Input 1 |");
        expect(mdExport).toContain("| +00:01 | Input | Input 2 |");
        expect(mdExport).toContain("| +00:02 | Input | Input 3 |");
        expect(mdExport).not.toContain("| System audio |");
      });
      ...
      it("handles conversation with only output entries", () => {
        ...
        const mdExport = buildExportText(entries, "md");
        expect(mdExport).toContain("| +00:00 | System audio | Output 1 |");
        expect(mdExport).toContain("| +00:01 | System audio | Output 2 |");
        expect(mdExport).not.toContain("| Input |");
      });
    });
  });
});
