...
function buildExportMarkdown(entries: HistoryEntry[], exportDate: string): string {
  ...
  if (outputEntries.length > 0) {
    ...
    outputEntries.forEach((entry) => {
      const time = formatTime(entry.timestamp_ms);
      const source = entry.source === "output" ? "Output" : "System";
...
function buildExportJson(entries: HistoryEntry[], exportDate: string): string {
  const exportData = {
...
const DEFAULT_TOPICS: TopicKeywords = {
